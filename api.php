...
if($_SERVER['REQUEST_METHOD']=='POST'){
	...
	try{
		...
		$maxidquery="SELECT MAX(id) FROM center";
		$table=$conn->query($maxidquery)->fetchAll();
		
		$newid=$table[0][0]+1;
		$name=$phparray['name'];
		$address=$phparray['address'];
		$testname=$phparray['testname'];
		$price=$phparray['price'];
		
		$insertquery="INSERT INTO center VALUES($newid,'$name','$address','$testname','$price')";
		$conn->exec($insertquery);
		
		http_response_code(201);
		echo json_encode(array("message"=>"added successfully", "id"=>$newid));
...
if($_SERVER['REQUEST_METHOD']=="PUT"){
	...
	if(isset($_GET['id'])){
		...
		try{
			...
			$updatequery="UPDATE center SET name='$name', address='$address', testname='$testname', price='$price' WHERE id='$id' ";
...
if($_SERVER['REQUEST_METHOD']=="DELETE"){
	...
	if(isset($_GET['id'])){
		...
		try{
			...
			$deletequery="DELETE FROM center WHERE id=$id ";
